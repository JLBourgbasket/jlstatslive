...
async function read(image, mediaType, table, quartersOnly, from, to) {
  ...
  if (quartersOnly) {
    return { quarters: data.quarters && data.quarters.team && data.quarters.opp ? data.quarters : null, model: MODEL, usage: api.usage || null };
  }
  return {
    team: data.team || null,
...
export default async (req) => {
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        const out = await read(image, mediaType, table, quartersOnly, from, to);
